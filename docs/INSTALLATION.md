# AIA Music Installation Guide

Complete guide to deploy AIA Music on your Hostinger VPS.
...
- Domain: music.aiacopilot.com pointing to your VPS IP
...
From your local machine, upload the AIA Music directory to your VPS:

```bash
# Using scp
scp -r AIA Music root@srv800338.hstgr.cloud:/var/www/
...
- Create database `aiamusic_db`
- Create user `aiamusic_user` with a generated password
...
Ensure your domain `music.aiacopilot.com` points to your VPS IP:
...
```bash
sudo certbot --nginx -d music.aiacopilot.com
```
...
- **Username:** admin
- **Password:** create through the approved admin setup path; do not use shared default passwords.
...
tail -f /srv/apps/aiamusic/logs/gunicorn_access.log

# Gunicorn error logs
tail -f /srv/apps/aiamusic/logs/gunicorn_error.log
