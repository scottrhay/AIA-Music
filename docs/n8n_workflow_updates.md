# n8n Workflow Updates for SunoApp

This document explains how to update your existing n8n workflow to work with the new MySQL-based SunoApp instead of Excel.

## Overview of Changes

Your current n8n workflow reads from Excel, submits to Suno, and updates Excel with results. The new workflow will:
1. Read songs with status "create" from MySQL database
2. Submit to Suno API
3. Update MySQL with "submitted" status and task ID
4. Receive webhook callback from Suno
...
### 4. Update Suno HTTP Request Node
...
**NEW:** HTTP Request to SunoApp webhook

**HTTP Request Node:**
- Method: POST
- URL: `https://suno.aiacopilot.com/api/v1/webhooks/suno-submitted`
- Headers:
...
**HTTP Request Node:**
- Method: POST
- URL: `https://suno.aiacopilot.com/api/v1/webhooks/suno-callback`
- Headers:
  - Content-Type: application/json
- Body:
```json
...
- **Option A:** Remove the MP3 download/upload nodes - users download directly from Suno URLs
- **Option B:** Keep them but update to read song data from MySQL instead of Excel

If keeping Option B, replace Excel read with:
```sql
SELECT * FROM songs WHERE suno_task_id = '{{ $('Webhook').first().json.body.data.task_id }}';
```
...
```
┌─────────────────────────────────────────────────────────────┐
│ WORKFLOW 1: Submit Songs to Suno                            │
├─────────────────────────────────────────────────────────────┤
│ 1. Schedule Trigger (every 5 minutes)                       │
│ 2. MySQL: Get songs WHERE status='create' LIMIT 10          │
│ 3. Loop Over Items                                          │
│ 4. HTTP Request: POST to Suno API                           │
│ 5. HTTP Request: POST to /webhooks/suno-submitted           │
│ 6. Loop back to step 3 until all items processed            │
└─────────────────────────────────────────────────────────────┘

┌─────────────────────────────────────────────────────────────┐
│ WORKFLOW 2: Handle Suno Callback                            │
├─────────────────────────────────────────────────────────────┤
│ 1. Webhook: Receive Suno completion callback                │
│ 2. If: Check if msg === "All generated successfully."       │
│ 3. HTTP Request: POST to /webhooks/suno-callback            │
...
GET https://suno.aiacopilot.com/api/v1/songs/pending-submission
Headers:
...
- **Fix:** Check Flask logs: `sudo journalctl -u sunoapp -f`
- Verify webhook URL is correct
- Check if SunoApp API is running

**Issue:** Songs stuck in "submitted" status
- **Fix:** Check if webhook callback is being received
- Verify callback URL in Suno API request
