...
```bash
# 1. Login
TOKEN=$(curl -X POST https://music.aiacopilot.com/api/v1/auth/login \
  -H "Content-Type: application/json" \
  -d '{"username":"admin","password":"REPLACE_WITH_ADMIN_PASSWORD"}' \
  | jq -r '.access_token')

# 2. Create song
curl -X POST https://music.aiacopilot.com/api/v1/songs \
  -H "Content-Type: application/json" \
...
```bash
curl -X GET "https://music.aiacopilot.com/api/v1/songs?status=completed&search=prayer" \
  -H "Authorization: Bearer $TOKEN"
```
...
```bash
curl -X GET "https://music.aiacopilot.com/api/v1/songs/stats?all_users=true" \
  -H "Authorization: Bearer $TOKEN"
```
