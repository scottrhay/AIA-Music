...
# AIA Music Database Setup Script (for Docker deployment)
...
CREATE USER IF NOT EXISTS 'aiamusic_user'@'%' IDENTIFIED BY '$DB_PASSWORD';
GRANT ALL PRIVILEGES ON aiamusic_db.* TO 'aiamusic_user'@'localhost';
GRANT ALL PRIVILEGES ON aiamusic_db.* TO 'aiamusic_user'@'%';
FLUSH PRIVILEGES;
EOF
