...
- **Audio files** - MP3 files stored in `/srv/apps/aiamusic/data/audio/`
...
| Audio | `/srv/apps/aiamusic/data/audio/` | `audio.tar.gz` |
| Config | `/srv/apps/aiamusic/.env` | `env.backup` |
...
scp deploy/backup.sh deploy/restore.sh deploy/BACKUP.md \
    root@168.231.71.238:/srv/apps/aiamusic/deploy/

# Make executable
ssh root@168.231.71.238 "chmod +x /srv/apps/aiamusic/deploy/*.sh"
```
...
# Add this line:
0 3 * * * DB_PASSWORD="your-db-password" /srv/apps/aiamusic/deploy/backup.sh >> /var/log/aiamusic-backup.log 2>&1
```
...
0 2 * * 0 DB_PASSWORD="your-db-password" RETENTION_DAYS=90 /srv/apps/aiamusic/deploy/backup.sh >> /var/log/aiamusic-backup.log 2>&1
```
...
2. Create a sync script `/srv/apps/aiamusic/deploy/sync-to-cloud.sh`:
```bash
#!/bin/bash
...
```bash
30 3 * * * /srv/apps/aiamusic/deploy/sync-to-cloud.sh >> /var/log/aiamusic-backup.log 2>&1
```
...
```bash
git clone <your-repo> /srv/apps/aiamusic
...
Create `/srv/apps/aiamusic/deploy/check-backup.sh`:
```bash
#!/bin/bash
...
```bash
0 9 * * * /srv/apps/aiamusic/deploy/check-backup.sh || mail -s "AIAMusic Backup Alert" you@email.com
```
...
```bash
sudo chown -R root:root /srv/apps/aiamusic/data
```
