...
# AIA Music Docker Deployment Script
...
echo "=== AIA Music Docker Deployment ==="
echo ""
...
if [ ! -f /etc/letsencrypt/live/music.aiacopilot.com/fullchain.pem ]; then
    ...
    if [[ $REPLY =~ ^[Yy]$ ]]; then
        ...
        echo "sudo certbot certonly --standalone -d music.aiacopilot.com"
    fi
fi
